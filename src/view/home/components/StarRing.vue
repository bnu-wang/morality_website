...
function deg2xyz(rx: number, ry: number, degree: number) {
    const x = rx * Math.cos(Math.PI / 180 * degree);
    const y = ry * Math.sin(Math.PI / 180 * degree);
    const z = 0;
    return [x, y, z];
}
function create_point_attr_pos() {
    // 为 x 轴和 y 轴分别设置不同的半径范围
    const rx = THREE.MathUtils.randFloat(9, 12);  // x 轴半径范围
    const ry = THREE.MathUtils.randFloat(4, 7);   // y 轴半径范围，比 x 轴小以形成椭圆
...
function init() {
    ...
    p_geometry.setAttribute('color', new THREE.Float32BufferAttribute(color, 4));
    (() => {
        const tpos: Array<number> = [];
        for (let i = 0; i < starData.length; i++) {
            const [rx, ry, degree] = [pos[i * 3 + 0], pos[i * 3 + 1], pos[i * 3 + 2]];
            const [x, y, z] = deg2xyz(rx, ry, degree);
            tpos.push(x, y, z);
        }
        p_geometry.setAttribute('position', new THREE.Float32BufferAttribute(tpos, 3));
    })();
    ...
    camera.position.set(5, -3, 5); // x y z
    camera.lookAt(5, -3, 0);
    ...
    function animate() {
        ...
        (() => {
            for (let step = 0; step < starData.length; step++) {
                pos[step * 3 + 2] = (pos[step * 3 + 2] + speed) % 360;
            }
            const tpos: Array<number> = [];
            for (let i = 0; i < starData.length; i++) {
                const [rx, ry, degree] = [pos[i * 3 + 0], pos[i * 3 + 1], pos[i * 3 + 2]];
